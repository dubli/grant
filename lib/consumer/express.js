...
function Grant (_config, additionalMiddleware) {
  ...
  if (additionalMiddleware) {
    additionalMiddleware = Array.isArray(additionalMiddleware) ? additionalMiddleware : [additionalMiddleware]
    for (var i in additionalMiddleware) {
      app.use(additionalMiddleware[i]);
    }
  }
  ...
  function connect (req, res) {
    ...
    if (provider.oauth == 1) {
      flow.step1(provider, function (err, data) {
        if (err) return res.redirect(provider.callback + '?' + appendStateToError(err, grant.dynamic))
        grant.step1 = data
