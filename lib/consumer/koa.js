...
function Grant (_config, additionalMiddleware) {
  var app = koa()
  ...
  if (additionalMiddleware) {
    if (Array.isArray(additionalMiddleware)) {
      additionalMiddleware.forEach(app.use)
    } else {
      app.use(additionalMiddleware)
    }
  }
  ...
  function* connect () {
    ...
    if (provider.oauth == 1) {
      ...
      try {
        ...
        return this.response.redirect(provider.callback + '?' + appendStateToError(err))
      }
      grant.step1 = data
      ...
      var err = {error:'Grant: missing or misconfigured provider', dynamic: grant.dynamic}
  ...
  function* callback () {
    ...
    var callback = function (response) {
      if (!provider.transport || provider.transport == 'querystring') {
        ...
        response.dynamic = grant.dynamic
    ...
    if (provider.oauth == 1) {
      try {
        ...
        return this.response.redirect(provider.callback + '?' + appendStateToError(err, grant.dynamic))
      }
      callback(response)
    }
      ...
      try {
        ...
        return this.reponse.redirect(provider.callback + '?' + appendStateToError(err, grant.dynamic))
      }
      callback(response)
    }

    else {
      var err = {error:'Grant: missing session or misconfigured provider', dynamic: grant.dynamic}
      if (provider.callback) {
        this.response.redirect(provider.callback + '?' + qs.stringify(err))
      } else {
        this.body = JSON.stringify(err)
...
function appendStateToError(err, state) {
  var original = qs.parse(err)
  original.dynamic = state
  return qs.stringify(original)
}
